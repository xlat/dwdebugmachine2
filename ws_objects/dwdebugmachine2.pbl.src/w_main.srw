...
event clicked;if NOT checked then
	if isValid( uo_powerfilter ) and not isNull( uo_powerfilter ) then
		//remove all pbni dependency... so that's dynamic stuff is not possbile anymore
...
//Another solution is to Instantiate the global w_genidwdm2_dwdebugger in order to 
//directly call the "Class Method" 
//E.g.: in the Application open script: 
If IsNull(w_genidwdm2_dwdebugger) Or Not Isvalid(w_genidwdm2_dwdebugger) then
	w_genidwdm2_dwdebugger = create w_genidwdm2_dwdebugger
end if
//Then, here we could write :
w_genidwdm2_dwdebugger.showdebugmsg( true )
//w_genidwdm2_dwdebugger lw_tmp
//lw_tmp = create w_genidwdm2_dwdebugger
...
w_genidwdm2_dwexpr_palette_dbg lw_pal2
openwithparm( lw_pal2, dw_1 )
setPointer(Arrow!)
end event
...
event clicked;w_genidwdm2_dwdebugger lw_modal2
openwithparm(lw_modal2, dw_1 )
end event
