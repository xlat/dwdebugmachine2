$PBExportHeader$nv_genidwdm2_auto_resizer.sru
$PBExportComments$Resize handler that can move / resize objects in a container during resizing - Author GeNi
forward
global type nv_genidwdm2_auto_resizer from nonvisualobject
end type
type rect from structure within nv_genidwdm2_auto_resizer
end type
end forward
...
global nv_genidwdm2_auto_resizer nv_genidwdm2_auto_resizer
...
nv_genidwdm2_auto_resizer ir_registered_controls[]
nv_genidwdm2_auto_resizer inv_parent_resizer
...
public function string of_datatypeof (powerobject apo_object);string ls_datatype
...
//return the y value of the currenct object
choose case of_datatypeof( apo_obj )
	case "window"
		window l_window
		l_window = apo_obj
...
if apo_obj.TypeOf() = window! then
	...
	//return the width value of the currenct object
	choose case of_datatypeof( apo_obj )
		case "window"
			window l_window
			l_window = apo_obj
			return l_window.width
		case "dragobject"
			dragobject l_object
			l_object = apo_obj
			return l_object.width
		case "roundrectangle"
			roundrectangle l_rr
			l_rr = apo_obj
			return l_rr.width
		case "line"
			line l_line
			l_line = apo_obj
			return abs(l_line.endx - l_line.beginx)
		case else
...
//		window l_window
//		l_window = apo_obj
//		return l_window.height
	case "dragobject"
		dragobject l_object
		l_object = apo_obj
		return l_object.height
	case "roundrectangle"
		roundrectangle l_rr
		l_rr = apo_obj
		return l_rr.height
	case "line"
		line l_line
		l_line = apo_obj
		return abs(l_line.endy - l_line.beginy)
	case else
		//debug_message( classname(), "of_get_height:  type not handled = " + of_datatypeof( apo_obj ) )
end choose
return 0
end function
...
//set the y value of the currenct object
choose case of_datatypeof( apo_obj )
	case "window"
		window l_window
		l_window = apo_obj
	...
	case "line"
		...
		l_line = apo_obj
		long ll_delta
		ll_delta = l_line.endy - l_line.beginy
		l_line.beginy = al_y
		l_line.endy = al_y + ll_delta
	case else
...
//set the x value of the currenct object
choose case of_datatypeof( apo_obj )
	case "window"
		window l_window
		l_window = apo_obj
	...
	case "line"
		...
		l_line = apo_obj
		long ll_delta
		ll_delta = l_line.endx - l_line.beginx
		l_line.beginx = al_x
		l_line.endx = al_x + ll_delta
	case else
...
//set the width value of the currenct object
choose case of_datatypeof( apo_obj )
	case "window"
		window l_window
		l_window = apo_obj
	...
	case "line"
		line l_line
		l_line = apo_obj
		l_line.endx = l_line.beginx + al_w
	case else
...
//set the height value of the currenct object
choose case of_datatypeof( apo_obj )
	case "window"
		window l_window
		l_window = apo_obj
	...
	case "line"
		line l_line
		l_line = apo_obj
		l_line.endy = l_line.beginy + al_h
	case else
		//debug_message( classname(), "of_set_hieght:  type not handled = " + of_datatypeof( apo_obj ) )
end choose

end subroutine

public function string of_datatypeofcontainer (powerobject apo_object);string ls_datatype
choose case apo_object.typeof()
	case commandbutton!, vprogressbar!, vtrackbar!, statichyperlink!, singlelineedit!, statictext!, &
		picturebutton!, checkbox!, datawindow!, radiobutton!, inkedit!, inkpicture!, animation!, &
		groupbox!, datepicker!, olecustomcontrol!, treeview!, listview!, graph!, monthcalendar!, &
...
long i
boolean lb_updated
nv_genidwdm2_auto_resizer lnv_resizer
...
public function boolean of_register_controller (powerobject apo_obj);if true /*of_isvisual( apo_obj ) and match( upper(of_get_tag( apo_obj )), "^[LRTB];?.*$" )*/ then
	nv_genidwdm2_auto_resizer lnv_resizer
	lnv_resizer = create nv_genidwdm2_auto_resizer
...
long i
nv_genidwdm2_auto_resizer lnv_res
...
public function string gettag (powerobject apo_obj);//return the tag value of the currenct object
string ls_tag
choose case of_datatypeof( apo_obj )
	case "window"
		window l_window
...
on nv_genidwdm2_auto_resizer.create
call super::create
TriggerEvent( this, "constructor" )
end on

on nv_genidwdm2_auto_resizer.destroy
